...
- The main menu's social links (the icon bar in the bottom-right corner): Tab reaches them as their own "Social links" group, left/right move between "View Steam store page" and "Join our Discord", and Enter opens the link in your browser - the game's own behavior for these buttons.
...
- Ctrl with Up/Down: jump by submenu. Ctrl+Down lands exactly where Down at the submenu's last item would (out to whatever follows, entering a directly following submenu); Ctrl+Up lands on the submenu's header, and pressing it again climbs out one level at a time.

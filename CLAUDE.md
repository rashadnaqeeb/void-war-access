...
- `src/gml/` - one module per script, one file each; build-mod globs
  `scr*.gml`, so a new script is just a new file (no build-code change).
  No file list here - list the directory and read the headers; every
  header opens with what its module is. Conventions: one `scrVwaMenu*`
  file per game screen family (a new family gets its own file);
  `scrVwaDev*` and `scrVwaTest*` are dev-build-only (the list lives in
  build-mod.csx's devOnlyScripts). `*.append.gml` appends to the named
  code entry (`*.dev.append.gml` = dev-only); `*.replace.gml` replaces
  one wholesale (build-mod asserts each replacement landed).
...
- **Smokes** (against a live game at the main menu), three scripts:
  ...
  screen script.** The assertions of `smoke` live IN THE GAME (the
  `scrVwaTest*` scripts): `vwa_dev_selftest` unit-tests the pure modules
  on fixtures, and the walker (`vwa_dev_walk_start`) sweeps every node of
  a screen, verifying real speech against a fresh live resolve. The PS
...
doc comment in the scrVwaDev*/scrVwaTest* scripts - the comment is its
spec, including the quirks. The testing entry points: `call vwa_dev_selftest` (pure-module
...
activates, **Tab/Shift+Tab** cycle control groups, **Alt+up/down** line
review, **letters** type-ahead search, **Escape** nav-back (consumed only
when the mod actually acts; otherwise the game's own Escape runs
untouched).

Each layer's full key map lives in its owning script's header - no key
list here beyond the core above (it drifts as features ship): screens in
...
- **One speech chokepoint.** All speech flows through `vwa_speak(parts,
  ...
  (dev builds: the test scripts' walkers); it observes, never speaks. No direct

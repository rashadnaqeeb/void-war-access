...
function vwa_dev_selftest()
{
    ...
    vwa_test_shipdesc(tc);
    return { checks: tc.checks, failures: tc.failures };
}

// Fixture adjacency for the ship describer: every neighboring tile pair
// passes except the given ["k1|k2"] blocked edges (order-normalized
// here) - the shape vwa_ship_geom_adj builds live.
function vwa_test_desc_adj(geom, blockedPairs)
{
    var blocked = {};
    for (var i = 0; i < array_length(blockedPairs); i++)
    {
        variable_struct_set(blocked, blockedPairs[i], true);
    }
    ...
    for (var i = 0; i < array_length(keys); i++)
    {
        ...
        {
            var nk = string(t.tx + dirs[d].dx) + ","
                + string(t.ty + dirs[d].dy);
            if (!variable_struct_exists(geom.tiles, nk))
            {
                continue;
            }
            var pairA = keys[i] + "|" + nk;
            var pairB = nk + "|" + keys[i];
            if (variable_struct_exists(blocked, pairA)
                || variable_struct_exists(blocked, pairB))
            {
                continue;
            }
            pass[d] = true;
...
// Fixture geom shorthand: tiles as [[tx, ty, cellId]] with slot centers
// on the 36px grid.
function vwa_test_desc_geom(tileDefs)
{
    var records = [];
    for (var i = 0; i < array_length(tileDefs); i++)
    {
        array_push(records, { sx: tileDefs[i][0] * 36,
            sy: tileDefs[i][1] * 36, cell: tileDefs[i][2], slot: 1 });
    }
    return vwa_ship_geom_build(records);
}

// The ship describer's pure pieces (scrVwaShipDesc): components,
// symmetry, segmentation and its cap fold, airlock sides, room counts,
// and line-level properties - all on constructed hulls, no live
// instances, no composed-string snapshots (the one spoken token
// compared is fetched through vwa_t at check time).
function vwa_test_shipdesc(tc)
{
    // Bar: one segment, symmetric, one component, room count by cell.
    var bar = vwa_test_desc_geom([[0, 0, "a"], [1, 0, "a"],
        [2, 0, "b"], [3, 0, "b"]]);
    var barAdj = vwa_test_desc_adj(bar, []);
    vwa_test_eq(tc, "shipdesc: bar sweeps horizontally",
        vwa_ship_desc_horiz(bar), true);
    var comps = vwa_ship_desc_components(bar, barAdj);
    vwa_test_eq(tc, "shipdesc: bar one component", array_length(comps), 1);
    vwa_test_eq(tc, "shipdesc: bar mismatch zero",
        vwa_ship_desc_mirror_mismatch(bar, true), 0);
    vwa_test_eq(tc, "shipdesc: bar rooms", vwa_ship_desc_room_count(bar, undefined), 2);
    var segs = vwa_ship_desc_segments(comps[0], true, 6);
    vwa_test_eq(tc, "shipdesc: bar one segment", array_length(segs), 1);
    vwa_test_ok(tc, "shipdesc: bar segment spans all",
        segs[0].u0 == 0 && segs[0].u1 == 3 && !segs[0].mixed,
        string(segs[0]));

    // Arrow: a 1-tall nose column then a 3-tall body; profile merge
    // keeps two segments, extents exact.
    var arrow = vwa_test_desc_geom([[0, 1, "n"],
        [1, 0, "b1"], [1, 1, "b2"], [1, 2, "b3"],
        [2, 0, "b4"], [2, 1, "b5"], [2, 2, "b6"]]);
    var arrowAdj = vwa_test_desc_adj(arrow, []);
    var acomps = vwa_ship_desc_components(arrow, arrowAdj);
    vwa_test_eq(tc, "shipdesc: arrow one component", array_length(acomps), 1);
    var asegs = vwa_ship_desc_segments(acomps[0], true, 6);
    vwa_test_eq(tc, "shipdesc: arrow two segments", array_length(asegs), 2);
    vwa_test_ok(tc, "shipdesc: arrow nose extent",
        asegs[0].u0 == 0 && asegs[0].u1 == 0
            && asegs[0].vMin == 1 && asegs[0].vMax == 1, string(asegs[0]));
    vwa_test_ok(tc, "shipdesc: arrow body extent",
        asegs[1].u0 == 1 && asegs[1].u1 == 2
            && asegs[1].vMin == 0 && asegs[1].vMax == 2, string(asegs[1]));
    vwa_test_eq(tc, "shipdesc: arrow symmetric about rows",
        vwa_ship_desc_mirror_mismatch(arrow, true), 0);

    // Cap fold: profiles with change costs 1, 2, 3 between neighbors -
    // the cap merges the SMALLEST change (the first pair), unions the
    // extent, and marks it mixed.
    var foldComp = [];
    var extents = [0, 1, 3, 6];
    for (var u = 0; u < 4; u++)
    {
        for (var v = 0; v <= extents[u]; v++)
        {
            array_push(foldComp, { tx: u, ty: v,
                key: string(u) + "," + string(v) });
        }
    }
    var fsegs = vwa_ship_desc_segments(foldComp, true, 3);
    vwa_test_eq(tc, "shipdesc: fold hits the cap", array_length(fsegs), 3);
    vwa_test_ok(tc, "shipdesc: fold merged the smallest change",
        fsegs[0].u0 == 0 && fsegs[0].u1 == 1 && fsegs[0].mixed
            && fsegs[0].vMin == 0 && fsegs[0].vMax == 1, string(fsegs[0]));
    vwa_test_ok(tc, "shipdesc: fold left the rest alone",
        !fsegs[1].mixed && !fsegs[2].mixed,
        string(fsegs[1]) + " " + string(fsegs[2]));
    vwa_test_eq(tc, "shipdesc: seg index inside",
        vwa_ship_desc_seg_index(fsegs, 2), 1);
    vwa_test_eq(tc, "shipdesc: seg index outside",
        vwa_ship_desc_seg_index(fsegs, 9), -1);

    // Components: a gap splits; a blocked edge splits; largest first,
    // equal sizes keep top-left seed order.
    var gap = vwa_test_desc_geom([[0, 0, "a"], [1, 0, "b"], [3, 0, "c"]]);
    var gcomps = vwa_ship_desc_components(gap, vwa_test_desc_adj(gap, []));
    vwa_test_eq(tc, "shipdesc: gap two components", array_length(gcomps), 2);
    vwa_test_eq(tc, "shipdesc: largest component first",
        array_length(gcomps[0]), 2);
    var wall = vwa_test_desc_geom([[0, 0, "a"], [1, 0, "b"]]);
    var wcomps = vwa_ship_desc_components(wall,
        vwa_test_desc_adj(wall, ["0,0|1,0"]));
    vwa_test_eq(tc, "shipdesc: blocked edge splits", array_length(wcomps), 2);
    vwa_test_ok(tc, "shipdesc: equal sizes keep seed order",
        wcomps[0][0].tx == 0, string(wcomps[0][0]));

    // Symmetry classes.
    vwa_test_eq(tc, "shipdesc: class sym", vwa_ship_desc_sym_class(0), "sym");
    vwa_test_eq(tc, "shipdesc: class rough at tolerance",
        vwa_ship_desc_sym_class(2), "rough");
    vwa_test_eq(tc, "shipdesc: class none past tolerance",
        vwa_ship_desc_sym_class(3), "none");
    var notch = vwa_test_desc_geom([[0, 0, "a"], [1, 0, "b"], [0, 1, "c"]]);
    vwa_test_eq(tc, "shipdesc: notch mismatch",
        vwa_ship_desc_mirror_mismatch(notch, true), 1);

    // Airlock sides on the bar: boundary edges name their outward
    // direction, a cell on both sides is interior.
    vwa_test_eq(tc, "shipdesc: airlock top",
        vwa_ship_desc_airlock_side(bar, 0, 0, 0), "top");
    vwa_test_eq(tc, "shipdesc: airlock bottom",
        vwa_ship_desc_airlock_side(bar, 1, 0, 1), "bottom");
    vwa_test_eq(tc, "shipdesc: airlock left",
        vwa_ship_desc_airlock_side(bar, 0, 0, 2), "left");
    vwa_test_eq(tc, "shipdesc: airlock right",
        vwa_ship_desc_airlock_side(bar, 3, 0, 3), "right");
    vwa_test_eq(tc, "shipdesc: airlock interior",
        vwa_ship_desc_airlock_side(bar, 1, 0, 3), "interior");

    // Line-level properties on the arrow: gestalt first with the
    // symmetry token (fetched live), every fixture system named in
    // exactly one line, the airlock line only when airlocks exist.
    var sys = [{ name: "fixSysA", tx: 0, ty: 1 },
               { name: "fixSysB", tx: 2, ty: 1 }];
    var lines = vwa_ship_desc_lines(arrow, arrowAdj, sys, []);
    vwa_test_eq(tc, "shipdesc: lines = gestalt + segments",
        array_length(lines), 3);
    var symTok = vwa_t("vwa--ship-desc-sym");
    var symSeen = 0;
    for (var i = 0; i < array_length(lines[0]); i++)
    {
        if (lines[0][i] == symTok)
        {
            symSeen += 1;
    ...
    vwa_test_eq(tc, "shipdesc: gestalt carries the symmetry token", symSeen, 1);
    for (var si = 0; si < array_length(sys); si++)
    {
        var hits = 0;
        for (var i = 0; i < array_length(lines); i++)
        {
            ...
            {
                if (lines[i][j] == sys[si].name)
                {
                    hits += 1;
                }
            }
        }
        vwa_test_eq(tc, "shipdesc: system " + sys[si].name
            + " named exactly once", hits, 1);
    }
    var lockLines = vwa_ship_desc_lines(arrow, arrowAdj, [],
        [{ tx: 0, ty: 1, dirIx: 2 }, { tx: 1, ty: 0, dirIx: 0 }]);
    vwa_test_eq(tc, "shipdesc: airlock line appended",
        array_length(lockLines), 4);
    var detach = vwa_ship_desc_lines(gap, vwa_test_desc_adj(gap, []), [], []);
    // gestalt + main segment + detached header + its segment.
    vwa_test_eq(tc, "shipdesc: detached section lines",
        array_length(detach), 4);
}
